...
const signalToRow = (s: SnapbackSignal) => ({
  ...
  strength:             s.tier ?? "REJECT",
  confidence:           Math.round(s.probabilityScore ?? 0),
  score:                Math.round((s.probabilityScore ?? 0) * 100),
  adjusted_score:       Math.round(
    ((s.layerScores?.compression ?? 0) + (s.layerScores?.expansion ?? 0)) * 50,
  ),
  duration_minutes:     2,
...
export const loadSignals = async (limit = 50): Promise<SnapbackSignal[]> => {
  try {
    ...
    return data.map((r): SnapbackSignal => {
      const prob = (Number(r.score) || 0) / 100;
      return {
        id:              r.id,
